...
void main() {
  querySelector('#BubbleSortClick').onClick.listen((event) {
    BubbleSortClick();
  });
  querySelector('#CaesarCipherClick').onClick.listen((event) {
    CaesarCipherClick();
  });
  querySelector('#IsPrimeClick').onClick.listen((event) {
    IsPrimeClick();
  });
  querySelector('#SerializeClick').onClick.listen((event) {
    SerializeClick();
  });
  querySelector('#Deserialize').onClick.listen((event) {
    Deserialize();
  });
  querySelector('#Generate').onClick.listen((event) {
    Generate();
  });
}
...
String CaesarCipher(text) {
  String zprava = "";
  for (int i = 0; i < text.length; i++) {
    int znak = text.codeUnitAt(i);
    znak += 1;
...
void IsPrimeClick() {
  double num =
      double.parse((querySelector("#strPrimeNumber") as InputElement).value);
  sw.start();
...
bool IsPrime(double number) {
  ...
  int boundary = (sqrt(number)).floor();
...
void SerializeClick() {
  Sklad sklad = new Sklad("Sklad Nice", "Broumov");
  sklad.Polozky = [
...
void Deserialize() {
  sw.start();
  Map skladMap = jsonDecode(Json);
  Sklad sklad = Sklad.fromJson(skladMap);
  sw.stop();
  print(sklad.Polozky.toString());
  print(sw.elapsedMilliseconds);
  sw.reset();
}
...
class Sklad {
  ...
  Sklad(String nazev, String misto) {
    this.Nazev = nazev;
    this.Misto = misto;
  }

  Sklad.fromJson(Map<String, dynamic> json)
      : Nazev = json['nazev'],
        Misto = json['misto'],
        Polozky = json['polozky'];

  Map<String, dynamic> toJson() =>
      {'nazev': Nazev, 'misto': Misto, 'polozky': Polozky};
}
...
class Polozka {
  ...
  Polozka(String nazev, String vyrobce, int cena, String popis) {
    this.Nazev = nazev;
  ...
  Polozka.fromJson(Map<String, dynamic> json)
      : Nazev = json['nazev'],
  ...
  Map<String, dynamic> toJson() =>
      {'nazev': Nazev, 'vyrobce': Vyrobce, 'popis': Popis, 'cena': Cena};
}
