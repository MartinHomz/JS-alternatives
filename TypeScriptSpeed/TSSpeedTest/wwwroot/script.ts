...
function CaesarCipherClick(): void {
    ...
    let start: number = new Date().getTime();
    let zprava: string = CaesarCipher(text);
    let end: number = new Date().getTime();
    console.log(zprava);
    console.log(end - start);
}
...
function CaesarCipher(text): string {
    ...
    for (let i = 0; i < text.length; i++) {
        let znak: number = text.charCodeAt(i);
        znak += 1;
...
function IsPrimeClick(): void {
    let num: number = parseInt((document.getElementById("strPrimeNumber") as HTMLInputElement).value);
    let start: number = new Date().getTime();
    let prime: boolean = IsPrime(num);
    let end: number = new Date().getTime();
    console.log(prime);
    console.log(end - start);
}
...
function SerializeClick(): void {
    let sklad: Sklad = new Sklad("Sklad Nice", "Broumov");
    sklad.Polozky = [
...
function Deserialize(): void {
    let start: number = new Date().getTime();
    let sklad:Sklad = JSON.parse(Json);
    let end: number = new Date().getTime();
...
class Sklad {
    public Nazev: string;
    public Misto: string;
    public Polozky: Polozka[];

    constructor(nazev: string, misto: string) {
        this.Nazev = nazev;
        this.Misto = misto;
    }
}
...
class Polozka {
    public Nazev: string;
    public Vyrobce: string;
    public Popis: string;
    public Cena: number;

    constructor(nazev: string, vyrobce: string, cena: number, popis: string) {
        this.Nazev = nazev;
